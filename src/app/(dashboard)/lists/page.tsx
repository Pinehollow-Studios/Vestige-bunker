import { CalendarClock, Clock, Hash, ListChecks, MapPin } from "lucide-react";
import { SectionHeader } from "@/components/admin/SectionHeader";
import { Badge } from "@/components/ui/badge";
import { activeStorageBaseUrl, tryCreateServiceClient } from "@/lib/supabase/admin";
import { avatarURL, listCoverURL } from "@/lib/storage";
import { QueueActions } from "./QueueActions";

export const dynamic = "force-dynamic";

/**
 * List-verification queue — public user lists awaiting the verified (Community)
 * stamp.
 *
 * Reads `user_lists` directly through the SERVICE-ROLE client: the old
 * `admin_list_verification_queue()` RPC was permanently dropped by the PII-drop
 * migration (`20260606140100`) because it referenced the removed
 * `users.first_name` column — so calling it 404s ("function … without
 * parameters in the schema cache"). The queue itself still matters (Community
 * lists), so we rebuild it here from the table: lists where
 * `verification_requested_at is not null and verified_at is null`, oldest first.
 * Approve / reject still go through the surviving `admin_*_user_list` RPCs.
 */

type CourseRow = {
  course_id: string;
  course_name: string;
  club_name: string | null;
  county_name: string | null;
  position: number | null;
};

type QueueRow = {
  list_id: string;
  list_name: string;
  list_description: string | null;
  privacy_kind: string;
  cover_url: string | null;
  created_at: string;
  verification_requested_at: string;
  owner_username: string;
  owner_display_name: string | null;
  owner_avatar_url: string | null;
  owner_bio: string | null;
  course_count: number;
  courses: CourseRow[];
};

type ServiceClient = NonNullable<Awaited<ReturnType<typeof tryCreateServiceClient>>>;

export default async function ListVerificationPage() {
  const supabase = await tryCreateServiceClient();
  if (!supabase) {
    return (
      <Shell>
        <div className="rounded-xl border border-amber/40 bg-amber/10 p-4 text-sm text-ink-2">
          List verification needs the service-role key for the active environment (the public-list
          queue isn&apos;t readable through the admin session). Set{" "}
          <code className="font-mono text-xs">SUPABASE_SERVICE_ROLE_KEY_PROD</code>.
        </div>
      </Shell>
    );
  }

  const baseUrl = await activeStorageBaseUrl();
  const { queue, error } = await loadQueue(supabase, baseUrl);

  return (
    <Shell>
      {error && (
        <div className="rounded-xl border border-alert/40 bg-alert/10 p-4 text-sm text-alert">
          Failed to load queue: {error}
        </div>
      )}

      {!error && queue.length === 0 && <EmptyQueue />}

      {queue.length > 0 && (
        <>
          <QueueSummary queue={queue} />
          <ol className="space-y-6">
            {queue.map((row, index) => (
              <li key={row.list_id}>
                <QueueCard row={row} position={index + 1} total={queue.length} />
              </li>
            ))}
          </ol>
        </>
      )}
    </Shell>
  );
}

async function loadQueue(
  supabase: ServiceClient,
  baseUrl: string,
): Promise<{ queue: QueueRow[]; error: string | null }> {
  const { data: lists, error } = await supabase
    .from("user_lists")
    .select(
      "id, name, description, privacy_kind, cover_storage_key, owner_user_id, verification_requested_at, created_at",
    )
    .not("verification_requested_at", "is", null)
    .is("verified_at", null)
    .order("verification_requested_at", { ascending: true });
  if (error) return { queue: [], error: error.message };

  const rows = (lists ?? []) as Array<{
    id: string;
    name: string;
    description: string | null;
    privacy_kind: string;
    cover_storage_key: string | null;
    owner_user_id: string;
    verification_requested_at: string;
    created_at: string;
  }>;
  if (rows.length === 0) return { queue: [], error: null };

  const ownerIds = Array.from(new Set(rows.map((r) => r.owner_user_id)));
  const listIds = rows.map((r) => r.id);

  const [ownersRes, coursesRes] = await Promise.all([
    supabase.from("users").select("id, username, display_name, avatar_photo_id, bio").in("id", ownerIds),
    supabase
      .from("user_list_courses")
      .select("user_list_id, position, courses(id, name, clubs(name), counties(name))")
      .in("user_list_id", listIds)
      .order("position", { ascending: true }),
  ]);

  const ownerById = new Map(
    ((ownersRes.data as Array<{ id: string; username: string; display_name: string | null; avatar_photo_id: string | null; bio: string | null }> | null) ?? []).map(
      (o) => [o.id, o] as const,
    ),
  );

  const coursesByList = new Map<string, CourseRow[]>();
  for (const jc of (coursesRes.data as Array<{ user_list_id: string; position: number | null; courses: unknown }> | null) ?? []) {
    const c = unwrap<{ id: string; name: string; clubs: unknown; counties: unknown }>(jc.courses);
    if (!c) continue;
    const list = coursesByList.get(jc.user_list_id) ?? [];
    list.push({
      course_id: c.id,
      course_name: c.name,
      club_name: unwrap<{ name: string }>(c.clubs)?.name ?? null,
      county_name: unwrap<{ name: string }>(c.counties)?.name ?? null,
      position: jc.position,
    });
    coursesByList.set(jc.user_list_id, list);
  }

  const queue: QueueRow[] = rows.map((r) => {
    const owner = ownerById.get(r.owner_user_id);
    const courses = coursesByList.get(r.id) ?? [];
    return {
      list_id: r.id,
      list_name: r.name,
      list_description: r.description,
      privacy_kind: r.privacy_kind,
      cover_url: listCoverURL(r.cover_storage_key, baseUrl),
      created_at: r.created_at,
      verification_requested_at: r.verification_requested_at,
      owner_username: owner?.username ?? "unknown",
      owner_display_name: owner?.display_name ?? null,
      owner_avatar_url: owner ? avatarURL(owner.id, owner.avatar_photo_id, baseUrl) : null,
      owner_bio: owner?.bio ?? null,
      course_count: courses.length,
      courses,
    };
  });

  return { queue, error: null };
}

function unwrap<T>(value: unknown): T | null {
  if (value == null) return null;
  if (Array.isArray(value)) return (value[0] as T | undefined) ?? null;
  return value as T;
}

// ── UI ─────────────────────────────────────────────────────────────────
function Shell({ children }: { children: React.ReactNode }) {
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <SectionHeader eyebrow="Operations · review" title="List verification" />
      {children}
    </div>
  );
}

function EmptyQueue() {
  return (
    <div className="flex flex-col items-center gap-3 rounded-xl glass-panel px-4 py-12 text-center">
      <span className="flex size-10 items-center justify-center rounded-full bg-brand/15 text-brand">
        <ListChecks className="size-5" aria-hidden />
      </span>
      <p className="font-display text-lg font-semibold text-ink">Queue is clear</p>
      <p className="text-sm text-ink-2">No public lists are awaiting the verified stamp.</p>
    </div>
  );
}

function QueueSummary({ queue }: { queue: QueueRow[] }) {
  const oldest = queue[0]?.verification_requested_at;
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl glass-panel px-4 py-3 text-xs text-ink-2">
      <div className="flex items-center gap-2">
        <span aria-hidden className="size-2 rounded-full bg-brand" />
        <span className="font-semibold text-ink">
          {queue.length} {queue.length === 1 ? "list" : "lists"} waiting
        </span>
        <span className="text-ink-3">· oldest first</span>
      </div>
      {oldest && (
        <span className="inline-flex items-center gap-1.5 text-ink-3">
          <CalendarClock aria-hidden className="size-3" />
          Top waiting <span className="font-medium text-ink">{formatRequested(oldest)}</span>
        </span>
      )}
    </div>
  );
}

function QueueCard({ row, position, total }: { row: QueueRow; position: number; total: number }) {
  const courses = row.courses;
  return (
    <article className="overflow-hidden rounded-xl glass-panel">
      <CoverBanner
        url={row.cover_url}
        title={row.list_name}
        position={position}
        total={total}
        waitingFor={formatRequested(row.verification_requested_at)}
      />
      <div className="flex flex-col gap-4 p-5">
        <header className="flex items-start gap-3">
          <div className="flex-1 space-y-1">
            <h2 className="font-display text-xl font-semibold leading-tight text-ink">{row.list_name}</h2>
            <p className="flex flex-wrap items-center gap-1.5 text-xs text-ink-3">
              <Clock aria-hidden className="size-3" />
              Requested {formatRequested(row.verification_requested_at)}
              <span aria-hidden>·</span>
              <span>created {formatDate(row.created_at)}</span>
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-1.5">
            <Badge variant="outline" className="border-brand/40 text-brand">
              {row.privacy_kind}
            </Badge>
            <Badge variant="outline" className="border-rule/70 text-ink-2">
              <Hash aria-hidden className="size-3" />
              {row.course_count} {row.course_count === 1 ? "course" : "courses"}
            </Badge>
          </div>
        </header>

        {row.list_description && (
          <section className="rounded-xl border border-rule/70 bg-paper-sunken/50 p-3 text-sm leading-relaxed text-ink-2">
            {row.list_description}
          </section>
        )}

        <OwnerBlock
          displayName={row.owner_display_name}
          username={row.owner_username}
          avatarURL={row.owner_avatar_url}
          bio={row.owner_bio}
        />

        <CourseList courses={courses} totalCount={row.course_count} />

        <footer className="flex justify-end pt-2">
          <QueueActions listId={row.list_id} listName={row.list_name} />
        </footer>
      </div>
    </article>
  );
}

function CoverBanner({
  url,
  title,
  position,
  total,
  waitingFor,
}: {
  url: string | null;
  title: string;
  position: number;
  total: number;
  waitingFor: string;
}) {
  return (
    <div className="relative aspect-video w-full overflow-hidden border-b border-rule/70 bg-paper-sunken">
      {url ? (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={url} alt={`Cover for ${title}`} className="h-full w-full object-cover" />
      ) : (
        <div className="flex h-full w-full items-center justify-center text-xs uppercase tracking-wider text-ink-3">
          No cover
        </div>
      )}
      <div className="absolute inset-x-0 top-0 flex items-start justify-between gap-2 p-3">
        <span className="rounded-full border border-rule/70 bg-paper-raised/85 px-2.5 py-1 text-xs font-semibold tabular-nums text-ink backdrop-blur-sm">
          {position} / {total}
          {position === 1 && total > 1 && (
            <span className="ml-1.5 text-[10px] font-semibold uppercase tracking-wider text-brand">Oldest</span>
          )}
        </span>
        <span className="rounded-full border border-rule/70 bg-paper-raised/85 px-2.5 py-1 text-xs text-ink-2 backdrop-blur-sm">
          Waiting <span className="font-semibold text-ink">{waitingFor}</span>
        </span>
      </div>
    </div>
  );
}

function OwnerBlock({
  displayName,
  username,
  avatarURL,
  bio,
}: {
  displayName: string | null;
  username: string;
  avatarURL: string | null;
  bio: string | null;
}) {
  const heading = displayName ?? username;
  const initials = ownerInitials(heading);
  return (
    <section className="flex items-start gap-3 rounded-xl border border-rule/70 bg-paper-sunken/40 p-3">
      {avatarURL ? (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={avatarURL} alt="" className="size-10 shrink-0 rounded-full bg-paper-sunken object-cover" />
      ) : (
        <div className="grid size-10 shrink-0 place-items-center rounded-full bg-brand/15 text-sm font-semibold text-brand">
          {initials}
        </div>
      )}
      <div className="min-w-0 flex-1 space-y-1">
        <p className="text-sm font-semibold leading-tight text-ink">
          {heading}
          <span className="ml-1.5 text-xs font-normal text-ink-3">@{username}</span>
        </p>
        {bio ? <p className="text-xs leading-snug text-ink-2">{bio}</p> : <p className="text-xs italic text-ink-3">No bio set</p>}
      </div>
    </section>
  );
}

function CourseList({ courses, totalCount }: { courses: CourseRow[]; totalCount: number }) {
  if (totalCount === 0) {
    return (
      <section className="rounded-xl border border-rule/70 p-3 text-center text-xs text-ink-3">
        No courses on this list.
      </section>
    );
  }
  return (
    <details className="group rounded-xl border border-rule/70 bg-paper-sunken/40">
      <summary className="flex cursor-pointer list-none select-none items-center justify-between gap-2 px-3 py-2 text-xs font-semibold text-ink-2">
        <span className="inline-flex items-center gap-1.5">
          <ListChecks aria-hidden className="size-3.5 text-brand" />
          Courses ({totalCount})
        </span>
        <span className="text-ink-3 group-open:hidden">Show all</span>
        <span className="hidden text-ink-3 group-open:inline">Hide</span>
      </summary>
      <ol className="divide-y divide-rule/60 border-t border-rule/70 text-sm">
        {courses.map((course, index) => (
          <li key={course.course_id} className="flex items-baseline gap-3 px-3 py-2">
            <span className="w-6 shrink-0 text-right text-xs tabular-nums text-ink-3">{course.position ?? index + 1}.</span>
            <div className="min-w-0 flex-1">
              <p className="truncate font-medium text-ink">{course.course_name}</p>
              <p className="flex items-center gap-1 truncate text-xs text-ink-3">
                {course.county_name && <MapPin aria-hidden className="size-3" />}
                {[course.club_name, course.county_name].filter(Boolean).join(" · ") || "—"}
              </p>
            </div>
          </li>
        ))}
      </ol>
    </details>
  );
}

function ownerInitials(source: string): string {
  const parts = source.trim().split(/\s+/).filter(Boolean);
  if (parts.length >= 2) return (parts[0][0] + parts[1][0]).toUpperCase();
  return (parts[0]?.slice(0, 2) ?? "?").toUpperCase();
}

function formatRequested(iso: string): string {
  const mins = Math.round((Date.now() - new Date(iso).getTime()) / 60_000);
  if (mins < 1) return "just now";
  if (mins < 60) return `${mins}m ago`;
  const hours = Math.round(mins / 60);
  if (hours < 24) return `${hours}h ago`;
  return `${Math.round(hours / 24)}d ago`;
}

function formatDate(iso: string): string {
  return new Date(iso).toLocaleDateString(undefined, { year: "numeric", month: "short", day: "numeric" });
}
